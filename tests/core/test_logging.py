...
class TestSamplingFilter:
    ...
    def test_stable_hash_deterministic(self):
        assert SamplingFilter._stable_hash("hello") == SamplingFilter._stable_hash("hello")
...
class TestLogContext:
    ...
    def test_context_sets_fields(self, caplog):
        # log_context uses ContextVars; _LogContextFilter injects them
        # into records going through the uasset_read logger
        ...
        with tempfile.TemporaryDirectory() as tmp:
            configure_project_logging(log_dir=Path(tmp), level="DEBUG")
            logger = logging.getLogger("uasset_read")
            with caplog.at_level(logging.DEBUG, logger="uasset_read"):
                with log_context(asset="BP_Player", stage="link"):
                    logger.info("inside context")
            record = caplog.records[-1]
            assert getattr(record, "asset", None) == "BP_Player"
            assert getattr(record, "stage", None) == "link"
            _reset_logging_state_for_tests()
