...
覆盖：异常类层次、tolerant_parse 去重、_handle_parse_error 各分支、
_record_parse_stage_error 去重与诊断。

注意：tolerant_parse 的基础场景（正常执行/捕获重抛/非容错传播）
已由 tests/core/test_error_handling.py 覆盖，本文件仅补充去重测试。
"""
...
# ===========================================================================
# 2. tolerant_parse — 去重（基础场景见 test_error_handling.py）
# ===========================================================================
...
# ===========================================================================
# 4. _record_parse_stage_error — 去重与诊断
# ===========================================================================
...
class TestRecordParseStageError:
    ...
    def test_different_exception_types_same_message_not_filtered(self):
        ...
        旧实现使用 str(error) 去重，导致不同异常类型但消息相同时
        第二个错误被错误过滤。新实现使用 (类型, 阶段, 消息) 元组去重。
        """
        result = _FakeResult()
        archive = _FakeArchive()
