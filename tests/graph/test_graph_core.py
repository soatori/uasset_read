"""Graph 核心测试 — 合并自 test_graph_quality.py、test_macro_expander.py、
test_subgraphs_and_fixes.py 和 test_standard_macro_cpp_mapping.py。

覆盖：图质量验证、宏展开、SubGraphs、控制流、标准宏 C++ 映射。
"""
...
class TestBuildGraphDictPinData:
    ...
    def test_pin_includes_id_and_linked_to_raw(self):
        ...
        graph = _make_macro_graph(name="G", guid="g1", nodes=[node])
    ...
    def test_pin_includes_parent_pin(self):
        ...
        node = _make_node(guid="node1", pins=[pin])
        graph = _make_macro_graph(name="G", nodes=[node])
...
class TestBuildGraphDictTunnelProps:
    ...
    def test_tunnel_node_has_exact_class_and_caps(self):
        ...
        graph = _make_macro_graph(name="G", nodes=[tunnel])
...
class TestBuildAssetContextSubgraphs:
    ...
    def test_top_level_graph_included(self):
        ...
        graph = _make_macro_graph(name="Main", guid="g1")
    ...
    def test_subgraph_included(self):
        ...
        child = _make_macro_graph(name="MacroGraph", guid="mg1")
        parent = _make_macro_graph(name="Parent", guid="p1", subgraphs=[child])
    ...
    def test_nested_subgraph_included(self):
        ...
        grandchild = _make_macro_graph(name="Deep", guid="d1")
        child = _make_macro_graph(name="Mid", guid="m1", subgraphs=[grandchild])
        parent = _make_macro_graph(name="Top", guid="t1", subgraphs=[child])
...
class _MacroPin:
    def __init__(self, name, direction, pin_category="exec", linked_to=None):
        self.pin_name = name
        self.direction = direction
        self.pin_type = _MacroPinType(pin_category)
        self.linked_to_raw = linked_to or []
        self.pin_id = f"PID_{name.upper()}"
...
def test_flow_penetrates_macro_instance():
    """执行链应穿透 MacroInstance 到其内部节点。"""
    event = _MacroFakeNode("guid_event", "K2Node_Event", [
        _MacroPin("exec", 1, "exec", ["PID_MACRO"]),
    ])
    macro = _MacroFakeNode("guid_macro", "K2Node_MacroInstance", [
        _MacroPin("exec", 0, "exec"),
        _MacroPin("Then", 1, "exec", ["PID_AFTER"]),
    ])
    after = _MacroFakeNode("guid_after", "K2Node_CallFunction", [
        _MacroPin("Then", 0, "exec"),
    ])
...
def test_standard_macro_marked():
    """标准宏（如 ForLoop）应被识别并标记为标准宏。"""
    event = _MacroFakeNode("guid_event", "K2Node_Event", [
        _MacroPin("exec", 1, "exec", ["PID_FORLOOP"]),
    ])
    forloop = _MacroFakeNode("guid_forloop", "K2Node_MacroInstance", [
        _MacroPin("exec", 0, "exec"),
        _MacroPin("Loop Body", 1, "exec", ["PID_AFTER"]),
    ], node_data={
        "macro_graph_reference": {
            "graph_name": "ForLoop",
            "graph_guid": "",
...
def test_async_action_marked_as_latent():
    ...
    event = _MacroFakeNode("guid_event", "K2Node_Event", [
        _MacroPin("exec", 1, "exec", ["B" * 32]),
    ])
    async_node = _MacroFakeNode("guid_async", "K2Node_AsyncAction", [
        _MacroPin("Then", 0, "exec"),
        _MacroPin("Completed", 1, "exec", ["D" * 32]),
    ])
    end_node = _MacroFakeNode("guid_end", "K2Node_MakeVariable", [
        _MacroPin("Completed", 0, "exec"),
    ])
...
def test_timeline_marked_as_latent():
    ...
    timeline = _MacroFakeNode("guid_timeline", "K2Node_Timeline", [
        _MacroPin("Update", 0, "exec"),
    ])
...
def test_normal_node_not_latent():
    ...
    call_func = _MacroFakeNode("guid_call", "K2Node_CallFunction", [
        _MacroPin("Then", 0, "exec"),
    ])
...
# ============================================================================
# 标准宏 → C++ 映射表测试（合并自 test_standard_macro_cpp_mapping.py）
# ============================================================================
...
# ==============================================================================
# 以下来自 test_subgraphs_and_fixes.py
# ==============================================================================
...
class TestSubGraphsParsing:
    ...
    def test_subgraphs_field_populated(self):
        ...
        child_graph = _sub_make_graph(name="ChildGraph")
        parent_graph = _sub_make_graph(name="ParentGraph", subgraphs=[child_graph])
    ...
    def test_nested_subgraphs(self):
        """嵌套子图（子图的子图）正确传递。"""
        grandchild = _sub_make_graph(name="Grandchild")
        child = _sub_make_graph(name="Child", subgraphs=[grandchild])
        parent = _sub_make_graph(name="Parent", subgraphs=[child])
    ...
    def test_ir_builder_preserves_subgraphs(self):
        ...
        child = _sub_make_graph(name="Child")
        parent = _sub_make_graph(name="Parent", subgraphs=[child])
    ...
    def test_subgraphs_with_mixed_sources(self):
        ...
        graph_a = _sub_make_graph(name="GraphA")
        graph_b = _sub_make_graph(name="GraphB")
        parent = _sub_make_graph(name="Parent", subgraphs=[graph_a, graph_b])
...
class TestSyntheticDataFlows:
    ...
    def _build_fe_node(
        ...
        """
        pins = [
            _sub_make_pin(f"fe_{i}", name, direction=dir, category=cat)
            for i, (name, dir, cat) in enumerate(pins_config)
        ]
        return _sub_make_node(
            guid="fe_guid_001",
            class_name="K2Node_FunctionEntry",
            pins=pins,
    ...
    def _build_call_node(
        ...
        for i, name in enumerate(input_pins):
            ...
            pins.append(_sub_make_pin(f"cf_in_{i}", name, direction=0, category="float", linked_to=linked))
        for i, name in enumerate(output_pins):
            pins.append(_sub_make_pin(f"cf_out_{i}", name, direction=1, category="float"))
        return _sub_make_node(
    ...
    def test_dynamic_detection_no_hardcoded_name(self):
        ...
        graph = _sub_make_graph(name="AnyCustomName", nodes=[fe, cf])
    ...
    def test_no_flows_without_function_entry(self):
        ...
        graph = _sub_make_graph(name="NoFE", nodes=[cf])
    ...
    def test_no_flows_without_matching_pins(self):
        ...
        graph = _sub_make_graph(name="NoMatch", nodes=[fe, cf])
    ...
    def test_already_connected_pins_excluded(self):
        ...
        graph = _sub_make_graph(name="Connected", nodes=[fe, cf])
...
# ==============================================================================
# 以下来自 test_standard_macro_cpp_mapping.py
# ==============================================================================
