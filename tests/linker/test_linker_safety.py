...
合并自：
- test_linker_offset_check.py: PackageLinker 偏移越界检查测试
- test_linker_safety.py: linker 安全修复 + UObjectInstance 测试

验证：
1. 越界 PackageIndex 返回 None 而非崩溃
2. 诊断信息包含 offset 和 file_size
3. serial_offset 溢出值（如 4294967296）被正确拦截
4. ObjectProperty/WeakObjectProperty 的 int 和 PackageIndex 值都能被正确解析
5. serial_size 负值被检测并标记失败
6. serial_size == 0 检查在偏移校验之后执行
7. UObjectInstance get_full_name() 正常路径、循环引用检测、边界情况
"""
...
# ======================================================================
# UObjectInstance get_full_name() 正常路径
# ======================================================================
...
# ======================================================================
# UObjectInstance get_full_name() 循环引用检测
# ======================================================================
