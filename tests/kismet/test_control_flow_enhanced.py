...
合并自 test_jump_analyzer.py：基础功能测试。
"""
...
# ─────────────────────────────────────────────────────────────────────────────
# 合并自 test_jump_analyzer.py 的基础功能测试
# ─────────────────────────────────────────────────────────────────────────────
...
class TestForDetection:
    ...
    def test_for_detection(self):
        ...
        jump_if_not = _make_jump_if_not_simple(statement_index=10, code_offset=50, boolean_expression=cond)
        body = _make_expr_simple(20)
        increment = _make_let_simple(30)
        jump_back = _make_jump_simple(statement_index=40, code_offset=10)
        exit_expr = _make_expr_simple(50)
    ...
    def test_for_with_multiple_increments(self):
        """多个递增表达式。"""
        cond = _make_expr_simple(0)
        jump_if_not = _make_jump_if_not_simple(statement_index=10, code_offset=60, boolean_expression=cond)
        body = _make_expr_simple(20)
        inc1 = _make_let_simple(30)
        inc2 = _make_let_simple(40)
        jump_back = _make_jump_simple(statement_index=50, code_offset=10)
        exit_expr = _make_expr_simple(60)
        exprs = [cond, jump_if_not, body, inc1, inc2, jump_back, exit_expr]
        analyzer = JumpAnalyzer(exprs)
    ...
    def test_for_body_too_short(self):
        ...
        cond = _make_expr_simple(0)
        jump_if_not = _make_jump_if_not_simple(statement_index=1, code_offset=20, boolean_expression=cond)
        # body_start=2, body_end=2 → body_end <= body_start → 不是 for
        jump_back = _make_jump_simple(statement_index=10, code_offset=1)
        exit_expr = _make_expr_simple(20)
        exprs = [cond, jump_if_not, jump_back, exit_expr]
        analyzer = JumpAnalyzer(exprs)
