...
| VectorCurves | TArray&lt;FVectorCurve&gt; | 向量曲线数组 (EditorOnly)，用于编辑器中的骨骼变换预览 | AnimCurveTypes.h 第 1082 行 |
| TransformCurves | TArray&lt;FTransformCurve&gt; | 变换曲线数组 (EditorOnly)，用于编辑器中的叠加动画编辑 | AnimCurveTypes.h 第 1088 行 |

说明：
- FloatCurves 在运行时和编辑器中都可用，用于驱动 Morph Target 权重和材质参数
- VectorCurves 和 TransformCurves 仅在编辑器中使用，运行时数据已烘焙到压缩数据中
...
| 字段名 | 类型 | 用途 | 源码位置 |
|--------|------|------|----------|
| CurveName | FName | 曲线名称，用于标识和查找 | AnimCurveTypes.h 第 112 行 |
...
| Name_DEPRECATED | FSmartName (EditorOnly) | 废弃：智能名称 | AnimCurveTypes.h 第 101 行 |

曲线类型标志 (EAnimAssetCurveFlags)：

| 标志名 | 值 | 用途 |
|--------|------|------|
| AACF_Editable | 0x00000004 | 可在序列编辑器中编辑 |
| AACF_Metadata | 0x00000010 | 元数据曲线，不参与运行时计算 |
| AACF_Disabled | 0x00000040 | 禁用曲线，不参与计算 |
| AACF_DriveTrack | 0x00000020 | 驱动骨骼轨道 (废弃) |

## FFloatCurve 浮点曲线字段

| 字段名 | 类型 | 用途 | 源码位置 |
|--------|------|------|----------|
| CurveName | FName (继承) | 曲线名称，通常对应 Morph Target 名称 | AnimCurveTypes.h 第 112 行 |
| FloatCurve | FRichCurve | 曲线数据，存储时间到值的映射 | AnimCurveTypes.h 第 218 行 |
| CurveTypeFlags | int32 (继承) | 曲线类型标志 | AnimCurveTypes.h 第 120 行 |

说明：
- 浮点曲线用于驱动 Morph Target 权重和材质参数
- CurveName 通常与 Morph Target 名称或材质参数名称对应
...
| 字段名 | 类型 | 用途 | 源码位置 |
|--------|------|------|----------|
| CurveName | FName (继承) | 曲线名称，对应骨骼名称 | AnimCurveTypes.h 第 112 行 |
...
| 字段名 | 类型 | 用途 | 源码位置 |
|--------|------|------|----------|
| CurveName | FName (继承) | 曲线名称 | AnimCurveTypes.h 第 112 行 |
...
| 字段名 | 类型 | 用途 | 源码位置 |
|--------|------|------|----------|
| Keys | TArray&lt;FRichCurveKey&gt; | 关键帧数组，按时间排序 | Curves/RichCurve.h |
| DefaultValue | float | 默认值，无关键帧时使用 | Curves/RichCurve.h |
| PreInfinityExtrap | ERichCurveExtrapolation | 前向外推模式，时间小于第一帧时的行为 | Curves/RichCurve.h |
| PostInfinityExtrap | ERichCurveExtrapolation | 后向外推模式，时间大于最后帧时的行为 | Curves/RichCurve.h |
...
| 字段名 | 类型 | 用途 | 源码位置 |
|--------|------|------|----------|
| Time | float | 时间点 (秒) | Curves/RichCurve.h |
| Value | float | 曲线值 | Curves/RichCurve.h |
| ArriveTangent | float | 到达切线，用于贝塞尔插值 | Curves/RichCurve.h |
| LeaveTangent | float | 离开切线，用于贝塞尔插值 | Curves/RichCurve.h |
| InterpMode | ERichCurveInterpMode | 插值模式 | Curves/RichCurve.h |
| TangentMode | ERichCurveTangentMode | 切线模式 | Curves/RichCurve.h |
| TangentWeightMode | ERichCurveTangentWeightMode | 切线权重模式 | Curves/RichCurve.h |
...
| 概念 | 说明 |
|------|------|
| TrackIndex | 动画轨道数组索引，对应 RawAnimationData[i] 或 AnimationTracks[i] |
...
3. RawAnimationData[TrackIndex] 存储该骨骼的位置/旋转/缩放关键帧

压缩数据索引：
- CompressedTrackToSkeletonMapTable 存储压缩数据的轨道到骨骼映射
- 压缩数据运行时使用此表快速定位骨骼变换
...
说明：
- 每个关键帧数组长度相同或与时间数组配合使用
- 数据存储骨骼相对于父骨骼的局部变换
- UE5 使用 FVector3f/FQuat4f (单精度)，UE4 使用 FVector/FQuat (双精度)
- UE5.0 后此结构已废弃，数据通过 FBoneAnimationTrack 管理
...
## 曲线类型分类 (EAnimCurveType)

| 类型 | 说明 | 用途 |
|------|------|------|
| AttributeCurve | 属性曲线 | 驱动动画属性 |
| MaterialCurve | 材质曲线 | 驱动材质参数 |
| MorphTargetCurve | Morph Target 曲线 | 驱动 Morph Target 权重 |
...
- Runtime/Engine/Public/Animation/AnimCurveTypes.h — FRawCurveTracks, FFloatCurve, FTransformCurve, FVectorCurve 定义
- Runtime/Engine/Public/Animation/AnimTypes.h — FTrackToSkeletonMap, FRawAnimSequenceTrack 定义
- Runtime/Core/Public/Curves/RichCurve.h — FRichCurve, FRichCurveKey 定义
