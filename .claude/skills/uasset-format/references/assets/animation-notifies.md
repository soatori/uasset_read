...
| Notify | UAnimNotify* | 单帧通知对象引用 | AnimTypes.h 第 301 行 |
| NotifyStateClass | UAnimNotifyState* | 持续通知对象引用 | AnimTypes.h 第 304 行 |
...
| MontageTickType | EMontageNotifyTickType::Type | 蒙太奇触发类型 | AnimTypes.h 第 318 行 |
| NotifyTriggerChance | float | 触发概率 (0=永不, 1=总是) | AnimTypes.h 第 322 行 |
| NotifyFilterType | ENotifyFilterType::Type | 过滤类型 | AnimTypes.h 第 326 行 |
...
| LinkedMontage | UAnimMontage* | 链接的蒙太奇 | AnimLinkableElement.h 第 147 行 |
| SlotIndex | int32 | 蒙太奇槽索引 | AnimLinkableElement.h 第 151 行 |
| SegmentIndex | int32 | 片段索引 | AnimLinkableElement.h 第 155 行 |
| LinkMethod | EAnimLinkMethod::Type | 时间链接方法 | AnimLinkableElement.h 第 159 行 |
| LinkValue | float | 链接时间值 | AnimLinkableElement.h 第 175 行 |
| LinkedSequence | UAnimSequenceBase* | 链接的动画序列 | AnimLinkableElement.h 第 182 行 |
...
| 字段名 | 类型 | 用途 | 源码位置 |
|--------|------|------|----------|
| LinkedMontage | UAnimMontage* | 链接的蒙太奇对象 | AnimLinkableElement.h 第 147 行 |
| SlotIndex | int32 | 蒙太奇槽索引 | AnimLinkableElement.h 第 151 行 |
| SegmentIndex | int32 | 片段索引 (INDEX_NONE 表示未链接) | AnimLinkableElement.h 第 155 行 |
| LinkMethod | EAnimLinkMethod::Type | 时间链接方法 | AnimLinkableElement.h 第 159 行 |
| CachedLinkMethod | EAnimLinkMethod::Type | 缓存的链接方法 | AnimLinkableElement.h 第 163 行 |
...
| MeshContext | USkeletalMeshComponent* | 当前触发的网格上下文 (私有) | AnimNotify.h 第 132 行 |

### 核心方法

| 方法名 | 说明 | 源码位置 |
|--------|------|----------|
| Notify() | 通知触发回调 (UE5.0 废弃) | AnimNotify.h 第 86 行 |
| Notify(EventReference) | 通知触发回调 (UE5.0+) | AnimNotify.h 第 87 行 |
| BranchingPointNotify() | 分支点通知回调 | AnimNotify.h 第 88 行 |
| GetNotifyName() | 获取自定义通知名称 | AnimNotify.h 第 59 行 |
| GetDefaultTriggerWeightThreshold() | 获取默认触发权重阈值 | AnimNotify.h 第 98 行 |

说明：
- bIsNativeBranchingPoint 为 true 时，蒙太奇中总是作为分支点处理
- Notify 方法接收 USkeletalMeshComponent 和 UAnimSequenceBase 参数
...
| 方法名 | 说明 | 源码位置 |
|--------|------|----------|
| NotifyBegin() | 状态开始回调 (UE5.0 废弃) | AnimNotifyState.h 第 65 行 |
| NotifyTick() | 状态帧回调 (UE5.0 废弃) | AnimNotifyState.h 第 67 行 |
| NotifyEnd() | 状态结束回调 (UE5.0 废弃) | AnimNotifyState.h 第 69 行 |
| NotifyBegin(EventReference) | 状态开始回调 (UE5.0+) | AnimNotifyState.h 第 71 行 |
| NotifyTick(EventReference) | 状态帧回调 (UE5.0+) | AnimNotifyState.h 第 72 行 |
| NotifyEnd(EventReference) | 状态结束回调 (UE5.0+) | AnimNotifyState.h 第 73 行 |
| BranchingPointNotifyBegin() | 分支点开始回调 | AnimNotifyState.h 第 75 行 |
| BranchingPointNotifyTick() | 分支点帧回调 | AnimNotifyState.h 第 76 行 |
| BranchingPointNotifyEnd() | 分支点结束回调 | AnimNotifyState.h 第 77 行 |
...
| 字段名 | 类型 | 用途 | 源码位置 |
|--------|------|------|----------|
| Notify | FAnimNotifyEvent* | 通知事件指针 | AnimNotifyQueue.h 第 122 行 |
| MirrorTable | UMirrorDataTable* | 镜像数据表 | AnimNotifyQueue.h 第 126 行 |
| NotifySource | UObject* | 通知来源对象 | AnimNotifyQueue.h 第 129 行 |
| CurrentAnimTime | float | 当前动画时间 | AnimNotifyQueue.h 第 132 行 |
| bActiveContext | bool | 是否为活动上下文 | AnimNotifyQueue.h 第 135 行 |
| ContextData | TArray&lt;TUniquePtr&lt;IAnimNotifyEventContextDataInterface&gt;&gt;* | 上下文数据数组 | AnimNotifyQueue.h 第 120 行 |

说明：
- NotifySource 是触发通知的动画序列或蒙太奇
- MirrorTable 用于镜像动画中的通知映射
- ContextData 存储额外的上下文信息
...
| AnimNotifies | TArray&lt;FAnimNotifyEventReference&gt; | 已触发的通知列表 | AnimNotifyQueue.h 第 206 行 |
| UnfilteredMontageAnimNotifies | TMap&lt;FName, FAnimNotifyArray&gt; | 未过滤的蒙太奇通知 | AnimNotifyQueue.h 第 210 行 |
| World | UWorld* | 世界指针 | AnimNotifyQueue.h 第 222 行 |
...
| 值 | 说明 | 源码位置 |
|----|------|----------|
| Queued | 通知排队到评估阶段结束后触发 (更快) | AnimTypes.h 第 91 行 |
...
- Runtime/Engine/Public/Animation/AnimTypes.h — FAnimNotifyEvent, FAnimSyncMarker, FAnimNotifyTrack 定义
- Runtime/Engine/Classes/Animation/AnimLinkableElement.h — FAnimLinkableElement 链接元素基类
- Runtime/Engine/Classes/Animation/AnimSequenceBase.h — UAnimSequenceBase 定义, Notifies 数组
...
| 特性 | 说明 |
|------|------|
| FAnimNotifyEventReference | 新增通知引用结构，传递更多上下文 |
