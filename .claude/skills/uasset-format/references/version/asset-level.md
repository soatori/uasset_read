...
| 版本号 | 变更描述 | 影响字段/结构 |
|-------|----------|---------------|
| 135 | World Level Info 引入 (VER_UE4_WORLD_LEVEL_INFO) | FWorldTileInfo |
| 225 | World Level Info 更新（父级引用、流送距离）(VER_UE4_WORLD_LEVEL_INFO_UPDATED) | ParentLevel, StreamingDistance |
| 321 | World Level Info LOD 列表 (VER_UE4_WORLD_LEVEL_INFO_LOD_LIST) | LODLevels 数组 |
| 327 | World Level Info ZOrder (VER_UE4_WORLD_LEVEL_INFO_ZORDER) | ZOrder |
| 343 | World 命名跟随包名 (VER_UE4_WORLD_NAMED_AFTER_PACKAGE) | UWorld 名称规则 |
| 347 | World Layer 启用距离流送 (VER_UE4_WORLD_LAYER_ENABLE_DISTANCE_STREAMING) | EnableDistanceStreaming |
| 464 | Level Streaming 绘制颜色类型变更 (VER_UE4_LEVEL_STREAMING_DRAW_COLOR_TYPE_CHANGE) | LevelStreamingColor |

### UE5 关卡版本变更

| 版本号 | 变更描述 | 影响字段/结构 |
|-------|----------|---------------|
| 1000 | UE5 初始版本 (EUnrealEngineObjectUE5Version::INITIAL_VERSION) | UE5 版本起点 |
| 1004 | Large World Coordinates (EUnrealEngineObjectUE5Version::LARGE_WORLD_COORDINATES) | FVector → double |
| 1017 | Verse Cells (EUnrealEngineObjectUE5Version::VERSE_CELLS) | Verse 相关 |
...
| 版本 | 变更 | 说明 |
|------|------|------|
| VER_UE4_LOW_QUALITY_DIRECTIONAL_LIGHTMAPS (183) | 低质量方向光照贴图 | Lightmap 格式 |
| VER_UE4_COMBINED_LIGHTMAP_TEXTURES (297) | 合并光照贴图纹理 | Lightmap 存储 |
| VER_UE4_REFLECTION_DATA_IN_PACKAGES (392) | 反射数据包存储 | ReflectionCapture |
...
| 版本 | 变更 | 说明 |
|------|------|------|
| VER_UE4_REBUILD_TEXTURE_STREAMING_DATA_ON_LOAD (241) | 加载时重建流数据 | StreamingData |
| VER_UE4_STREAMABLE_TEXTURE_MIN_MAX_DISTANCE (692) | MinMax Distance | Distance 流送 |
| VER_UE4_STREAMABLE_TEXTURE_AABB (699) | AABB 流送 | Bounds 流送 |
