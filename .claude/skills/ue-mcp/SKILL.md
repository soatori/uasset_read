...
The tracked source lives at `skills/ue-mcp/`. The Claude Code runtime copy lives at `.claude/skills/ue-mcp/`. Keep both copies synchronized after edits.
...
Currently enabled Toolsets are only the live `list_toolsets` result, not every plugin found in the engine source. In the current FirstPerson MCP session, the enabled set is the base editor stack: `ToolsetRegistry.AgentSkillToolset`, `EditorToolset.EditorAppToolset`, `EditorToolset.LogsToolset`, `AutomationTestToolset.AutomationTestToolset`, `LiveCodingToolset.LiveCodingToolset`, and `editor_toolset.toolsets.*` Python tools.
