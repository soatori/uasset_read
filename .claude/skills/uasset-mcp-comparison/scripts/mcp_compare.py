...
def main():
    import argparse
    parser = argparse.ArgumentParser(description='uasset MCP 对比')
    parser.add_argument('--project', required=True, help='项目名称')
    parser.add_argument('--blueprints', help='蓝图文件名（逗号分隔）')
    parser.add_argument('--output', help='输出文件路径')
    args = parser.parse_args()

    project_path = SAMPLES_ROOT / args.project
    if not project_path.exists():
        print(f'项目不存在: {project_path}')
        return

    if args.blueprints:
        bp_names = args.blueprints.split(',')
        bp_files = []
        for name in bp_names:
            found = list(project_path.glob(f'**/{name}.uasset'))
            if found:
                bp_files.append(found[0])
    else:
        bp_files = list(project_path.glob('**/BP_*.uasset'))[:15]

    client = McpClient()
    client.connect()
    print('MCP 连接成功')

    result = compare_project(args.project, bp_files, client)

    output_file = Path(args.output) if args.output else Path(f'temp/{args.project}-mcp-comparison-report.json')
