...
UE5 版本演进通过独立的 `EUnrealEngineObjectUE5Version` 枚举管理，版本号从 `INITIAL_VERSION = 1000` 开始，与 UE4 版本号分离，避免冲突。双版本机制允许 UE4 代码库变更与 UE5 代码库独立演进，同时保持包文件互操作性。
...
以下选取约 20 个 UE5 关键版本，标注变更描述和影响的资产类型。
...
| AUTO | AUTOMATIC_VERSION | 版本锚点，自动计算为最新 UE5 版本 | 所有资产 |
