...
BulkData 支持多种压缩格式，通过 EBulkDataFlags 标志位指定：

| 标志 | 压缩格式 | 说明 |
|------|----------|------|
| BULKDATA_SerializeCompressedZLIB | ZLIB | 标准 ZLIB 压缩，通用性好 |
| BULKDATA_SerializeCompressedLZ4 | LZ4 | LZ4 快速压缩（UE5 已废弃） |
| BULKDATA_SerializeCompressedOodle | Oodle | Oodle 压缩，Cooked 资产使用 |

### 压缩处理流程

1. **保存时**：根据压缩标志选择压缩算法，压缩数据后写入文件
2. **加载时**：Lock() 时检测压缩标志，调用对应解压器解压数据
...
| 标志 | 策略 | 说明 |
|------|------|------|
| BULKDATA_SingleUse | 单次使用 | Unlock 后释放内存 |
| BULKDATA_MemoryMappedPayload | 内存映射 | 使用文件映射而非 malloc |
| BULKDATA_DataIsMemoryMapped | 运行时标记 | 表示当前数据是内存映射 |
...
| 特性 | 说明 |
|------|------|
| PayloadTOC 支持 | 大数据分离存储，通过 PackageTrailer 管理 |
...
| 标志 | 状态 | 替代方案 |
|------|------|----------|
| BULKDATA_SerializeCompressedLZ4 | UE5 废弃 | 使用 Oodle 或 ZLIB |
