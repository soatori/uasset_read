...
| SystemUpdateMode | EParticleSystemUpdateMode | RealTime | 更新模式（RealTime/FixedTime） |
| LODMethod | ParticleSystemLODMethod | Automatic | LOD 判定方法（Automatic/DirectSet/ActivateAutomatic） |
| InsignificantReaction | EParticleSystemInsignificanceReaction | - | 全部发射器不活跃时的系统反应 |
| OcclusionBoundsMethod | EParticleSystemOcclusionBoundsMethod | ParticleBounds | 遮挡判定方法（None/ParticleBounds/CustomBounds） |
...
| bIsElligibleForAsyncTick | uint8:1 | 是否可异步 Tick（计算值） |
