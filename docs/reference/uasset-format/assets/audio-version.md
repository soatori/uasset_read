...
| 字段 | 类型 | 用途 | 源码位置 |
|------|------|------|----------|
| SoundAssetCompressionType | ESoundAssetCompressionType | 压缩格式枚举 (替代旧布尔属性) | SoundWave.h:463 |
| LoadingBehavior | ESoundWaveLoadingBehavior | 加载行为控制 (RetainOnLoad, PrimeOnLoad, LazyOnDemand) | SoundWave.h:743 |
| ZerothChunkData | FBulkDataBuffer<uint8> | 首块缓存优化 | SoundWave.h |
| bEnableCloudStreaming | bool | 云流式播放支持 | SoundWave.h |
| CuePoints | TArray<FCuePoint> | WAV 文件 Cue 点和 Loop Region 支持 | SoundWave.h |
| ESoundWaveCuePointOrigin | enum | Cue 点来源控制 | SoundWave.h:73-77 |
...
| UE4 属性 | UE5 替代 | 说明 |
|----------|----------|------|
| bUseBinkAudio | SoundAssetCompressionType::BinkAudio | 布尔属性改为枚举 |
| bSeekableStreaming | SoundAssetCompressionType::ADPCM (5.0 已弃用) | 流式 Seek 属性重构 |
| CompressionName | SoundAssetCompressionType | FName 改为枚举 |

### 数据存储变更

| 变更类型 | UE4 | UE5 |
|----------|-----|-----|
| RawData 存储 | FByteBulkData | FEditorAudioBulkData |
| CookedPlatformData 类型 | TMap | TSortedMap |
| 流式缓存 | 基础流式加载 | Stream Caching 机制 |
| 云流式播放 | 不支持 | bEnableCloudStreaming 支持 |

### 新增功能

| 功能 | UE4 | UE5 |
|------|-----|-----|
| RADAudio 格式 | 不支持 | 支持，高质量 BinkAudio |
| ProjectDefined 格式 | 不支持 | 使用项目默认编解码器 |
| Cue 点解析 | 不支持 | 从 WAV 文件解析 Cue Points/Loop Region |
| Cook-time 分析数据 | 不支持 | FFT/Envelope 数据支持 |
| 首块缓存优化 | 不支持 | ZerothChunkData 缓存 |
...
| DTYPE_Xenon | UE4 早期 | Xenon 平台已弃用 |
| bUseBinkAudio | UE5 | 整合到 SoundAssetCompressionType |
| bSeekableStreaming | UE5 | 整合到 SoundAssetCompressionType |
