...
| 字段名 | 类型 | 默认值 | 用途 |
|--------|------|--------|------|
| EmitterName | FName | NAME_None | 发射器名称 |
| SubUVDataOffset | int32 | - | (transient) SubUV 数据偏移，运行时计算 |
| EmitterRenderMode | EEmitterRenderMode | Normal | 渲染模式（Normal/Point/Cross/LightsOnly/None） |
| SignificanceLevel | EParticleSignificanceLevel | - | 显著性等级，决定发射器何时活跃 |
| bUseLegacySpawningBehavior | uint8:1 | false | 使用旧版粒子生成行为 |
| ConvertedModules | uint8:1 | - | 模块已转换标记 |
| bIsSoloing | uint8:1 | false | (transient) 编辑器独奏模式标记 |
| bCookedOut | uint8:1 | false | Cook 后剔除标记 — 详见下文 |
| bDisabledLODsKeepEmitterAlive | uint8:1 | false | 禁用 LOD 时是否保持发射器活跃 |
| bDisableWhenInsignficant | uint8:1 | false | 不活跃时立即禁用 Tick 和渲染 |
| bRemoveHMDRollInVR | uint8:1 | false | VR 中移除 HMD Roll |
| LODLevels | TArray&lt;TObjectPtr&lt;UParticleLODLevel&gt;&gt; | [] | LOD 层级数组，instanced 子对象 |
| PeakActiveParticles | int32 | - | 峰值活跃粒子数（计算值） |
| InitialAllocationCount | int32 | - | 初始分配计数（覆盖峰值计算，&gt;0 时生效） |
| QualityLevelSpawnRateScale | float | - | 品质等级生成率缩放因子 |
| DetailModeBitmask | uint32 | - | 细节模式位掩码（Low/Medium/High/Epic） |

### Transient 计算字段（磁盘上不序列化）

| 字段名 | 类型 | 用途 |
|--------|------|------|
| bRequiresLoopingNotification | uint8:1 | 需要循环通知（PostLoad 计算） |
| bAxisLockEnabled | uint8:1 | 轴锁定启用（PostLoad 计算） |
| bMeshRotationActive | uint8:1 | 网格旋转活跃（PostLoad 计算） |
| LockAxisFlags | EParticleAxisLock | 轴锁定标志（运行时计算） |

### WITH_EDITORONLY_DATA 字段

| 字段名 | 类型 | 用途 |
|--------|------|------|
| bCollapsed | uint8:1 | Cascade 编辑器中折叠显示 |
| EmitterEditorColor | FColor | 曲线编辑器和调试渲染颜色 |
...
| 字段名 | 类型 | 默认值 | 用途 |
|--------|------|--------|------|
| Level | int32 | - | LOD 层级索引（0=最高精度） |
| bEnabled | uint32:1 | true | 是否启用此 LOD 层级 |
| RequiredModule | TObjectPtr&lt;UParticleModuleRequired&gt; | - | 必需模块，每个 LODLevel 必须有，instanced |
| Modules | TArray&lt;TObjectPtr&lt;UParticleModule&gt;&gt; | [] | 模块数组，包含所有行为模块，instanced |
| TypeDataModule | TObjectPtr&lt;UParticleModuleTypeDataBase&gt; | - | 类型数据模块，定义发射器类型（Sprite/Mesh/Beam/Trail/GPU），export |
| SpawnModule | TObjectPtr&lt;UParticleModuleSpawn&gt; | - | 生成模块，所有发射器必需，export |
| EventGenerator | TObjectPtr&lt;UParticleModuleEventGenerator&gt; | - | 事件生成模块（可选），export |
| ConvertedModules | uint32:1 | - | 模块已转换标记 |
| PeakActiveParticles | int32 | - | 此 LOD 的峰值活跃粒子数 |

### Transient 数组（PostLoad 时重建）

以下数组在磁盘上不序列化，PostLoad 时从 Modules 数组重建：

| 字段名 | 类型 | 用途 |
|--------|------|------|
| SpawningModules | TArray&lt;TObjectPtr&lt;UParticleModuleSpawnBase&gt;&gt; | 决定生成多少粒子 |
| SpawnModules | TArray&lt;TObjectPtr&lt;UParticleModule&gt;&gt; | 粒子生成时调用 |
| UpdateModules | TArray&lt;TObjectPtr&lt;UParticleModule&gt;&gt; | 粒子更新时调用 |
| OrbitModules | TArray&lt;TObjectPtr&lt;UParticleModuleOrbit&gt;&gt; | 轨道偏移模块 |
| EventReceiverModules | TArray&lt;TObjectPtr&lt;UParticleModuleEventReceiverBase&gt;&gt; | 事件接收模块 |
...
| DetailModeBitmask | 无枚举关联 | 关联 EParticleDetailMode 枚举 |
