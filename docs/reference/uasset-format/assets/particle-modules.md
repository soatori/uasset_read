...
| 字段名 | 类型 | 默认值 | 用途 |
|--------|------|--------|------|
| bSpawnModule | uint8:1 | - | 在粒子生成时执行 |
| bUpdateModule | uint8:1 | - | 在粒子更新时执行 |
| bFinalUpdateModule | uint8:1 | - | 在最终更新时执行 |
| bUpdateForGPUEmitter | uint8:1 | - | 对 GPU 发射器执行 |
| bCurvesAsColor | uint8:1 | false | FVector 曲线显示为颜色 |
| b3DDrawMode | uint8:1 | false | 启用 3D 可视化绘制 |
| bSupported3DDrawMode | uint8:1 | - | 支持 3D 可视化 |
| bEnabled | uint8:1 | true | 模块是否启用 |
| bEditable | uint8:1 | true | 模块是否可编辑 |
| LODDuplicate | uint8:1 | - | LOD 自动生成时为精确副本 |
| bSupportsRandomSeed | uint8:1 | - | 支持随机种子设置 |
| bRequiresLoopingNotification | uint8:1 | - | 需要发射器循环通知 |
| LODValidity | uint8 | 0xFF | LOD 有效性位标志（(1&lt;&lt;Level) &amp; LODValidity != 0 表示在该 LOD 中有效） |

### WITH_EDITORONLY_DATA 字段

| 字段名 | 类型 | 用途 |
|--------|------|------|
| ModuleEditorColor | FColor | 曲线编辑器颜色 |
