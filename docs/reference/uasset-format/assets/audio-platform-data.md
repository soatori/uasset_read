...
运行时流式音频的平台特定数据，存储音频块数组及格式信息。

| 字段名 | 类型 | 用途 | 源码位置 |
|--------|------|------|----------|
| AudioFormat | FName | 音频块存储格式 | SoundWave.h:147 |
| Chunks | TIndirectArray<FStreamedAudioChunk> | 音频块数组 | SoundWave.h:149 |
| DerivedDataKey | FString | DDC 关联键 (EditorOnly) | SoundWave.h:153 |
| AsyncTaskLock | TDontCopy<FRWLock> | 异步任务锁 (EditorOnly) | SoundWave.h:156 |
| AsyncTask | FStreamedAudioAsyncCacheDerivedDataTask* | 异步缓存任务 (EditorOnly) | SoundWave.h:158 |
...
| GetChunks | TIndirectArray<FStreamedAudioChunk>& | 获取音频块数组 |
| GetNumChunks | int32 | 获取音频块数量 |
| GetAudioFormat | FName | 获取音频格式 |
| Serialize | void | 序列化操作 |
| Cache | void | 缓存平台数据 (EditorOnly) |

## FStreamedAudioChunk 结构

流式音频的单个数据块，支持按需加载和 Seek 定位。

| 字段名 | 类型 | 用途 | 源码位置 |
|--------|------|------|----------|
| DataSize | int32 | 数据大小 (含零填充) | SoundWave.h:104 |
| AudioDataSize | int32 | 音频数据大小 (含 Seek 表) | SoundWave.h:107 |
| SeekOffsetInAudioFrames | uint32 | 流中位置 (帧) | SoundWave.h:110 |
| BulkData | FByteBulkData | Bulk 数据存储 | SoundWave.h:113 |
| CachedDataPtr | uint8* | 缓存数据指针 (内部) | SoundWave.h:116 |
| DerivedDataKey | FString | DDC 键 (EditorOnly) | SoundWave.h:122 |
| bLoadedFromCookedPackage | bool | 是否从 cooked 包加载 (EditorOnly) | SoundWave.h:125 |
| bInlineChunk | bool | 是否内联块 (EditorOnly) | SoundWave.h:128 |
...
| Serialize | void | 序列化操作 |
| GetCopy | bool | 获取数据副本 |
| MoveOutAsBuffer | FBulkDataBuffer<uint8> | 移出数据为缓冲区 |
...
- 保证 16 位深度
- 单声道或立体声格式
- 多通道通过 ChannelOffsets/ChannelSizes 分离
...
- Runtime/Engine/Private/Sound/SoundWave.cpp — 平台数据序列化实现
...
- 替代传统 FByteBulkData
- 支持 Payload 机制和更灵活的数据管理

**流式缓存优化:**
- ZerothChunkData 首块缓存
- LoadOnDemand 按需加载机制
- 多种 LoadingBehavior 模式

**云流式播放:**
- bEnableCloudStreaming 支持
- PlatformSettings 平台特定设置
- 需平台插件支持
...
**UE5:**
- TSortedMap 优化平台数据索引
- FEditorAudioBulkData 支持 Payload
- 流式缓存机制 (Stream Caching)
- 云流式播放支持
- Cook-time 分析数据 (FFT/Envelope)
