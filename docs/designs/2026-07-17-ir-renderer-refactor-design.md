...
依赖优先：P1 → P2 → P8 → P9 → P3 → P4 → P5 → P6 → P7
...
- `src/uasset_read/renderers/markdown_renderer.py` — 渲染逻辑
- `tests/` — 相关测试用例
...
### 替换映射
- `imports: list[dict]` → `list[ImportIR]`
- `function_graphs: list[dict]` → `list[FunctionGraphIR]`
- `resolved_depends_map: list[list[dict]]` → `list[list[ResolvedDependIR]]`
- `ExportIR.diagnostics: dict` → `ExportDiagnosticsIR`
...
├── ir_anim.py     # 动画 IR 模型（AnimBlueprintIR、AnimSequenceIR 等）
└── __init__.py    # 统一导出
```
...
# ir_anim.py 导出动画模型
from .ir_anim import AnimBlueprintIR, AnimSequenceIR, ...
...
class ExportParseStatus(Enum):
    ...
    def is_partial(self) -> bool:
        return self in _PARTIAL_STATUSES
    
    @property
    def is_failed(self) -> bool:
        return self in _FAILED_STATUSES

# 自动生成集合（延迟定义）
_PARTIAL_STATUSES = frozenset(s for s in ExportParseStatus if s.value.startswith("partial"))
_FAILED_STATUSES = frozenset(s for s in ExportParseStatus if s.value.startswith("failed"))
```
...
- `src/uasset_read/models/status.py` — 状态集合
...
1. 将 `SKIP_CLASS_NAMES` 和 `SKIP_CLASS_PREFIXES` 中的条目迁移到 `CLASS_STRATEGY_TABLE`
2. 删除 `class_specific_skip.py` 中的冗余定义
3. 保留 `class_specific_skip.py` 作为查询接口（如有需要）
...
### 影响范围
- `src/uasset_read/parsers/class_serialization_strategy.py` — 策略表
...
- 高频 dict 字段替换为类型安全的 dataclass
- 动画 IR 模型独立到专用文件
- 消除重复代码和双重系统
- parse_uasset.py 职责更清晰
