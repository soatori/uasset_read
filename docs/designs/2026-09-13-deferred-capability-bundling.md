...
| D-BATCH | CLI `--batch` | [`2026-08-31-v1-retirement-plan.md`](2026-08-31-v1-retirement-plan.md) §5 | deferred; awaits product decision |
...
| D-BATCH ↔ D-DIFF | soft | Same deferred product decision, same CLI surface, parse entrypoints only. `batch_worker.py` was deleted with v1; any v2 batch is fresh orchestration. **Seed confirmed.** |
...
| 3 — Workflow CLI | D-BATCH + D-DIFF | Same CLI surface decision and same product-need gate; pure workflow, never blocks format work (rule 2). | Explicit user product need for batch orchestration and/or schema-compare diff | None (works on existing loose fixtures) | Whenever product asks |
| 4 — Perf | D-G2 | Free-standing cheap win (rule 3); S2 Route B rides it for free; contract already frozen with no API change. | A real multi-extract consumer path (e.g. MCP session) — already present | None | Anytime; may land before or after Bundle 1 |
...
- **D-BATCH / D-DIFF ∉ any format bundle** (rule 2: pure workflow never blocks format work).
...
1. Is there a real multi-file orchestration consumer today for `--batch`, or is it aspirational?
