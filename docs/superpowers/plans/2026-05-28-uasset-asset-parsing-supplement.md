...
import pytest
from uasset_read.parsers.ftext import read_ftext_with_history
...
class MockArchive:
    ...
    def read_fstring(self):
        # Simplified: read length-prefixed ANSI string
        length = self.read_i32()
        if length == 0:
            return ""
        data = self._data[self._pos:self._pos + abs(length) - 1]
        self._pos += abs(length)
        return data.decode('utf-8') if length > 0 else ""

def test_ordered_format_history_type():
    """history_type=2 (OrderedFormat) 应正常读取而非抛异常。"""
    # 模拟: history_type=2, source_count=1, args_count=2
    # source: {Namespace, Key, SourceString}
    # args: [arg0_name, arg0_value, arg1_name, arg1_value]
    data = bytes([2])  # history_type = OrderedFormat
...
修改 `src/uasset_read/parsers/ftext.py`，补全各 history_type：
...
```bash
git add src/uasset_read/parsers/ftext.py tests/phase76/test_ftext.py
...
def test_property_tag_ctrl_bits():
    ...
    # 验证 ctrl & 0x01 = 可覆盖序列化标志
    # 验证 ctrl & 0x02 = 序列化控制扩展
    from uasset_read.serializers.property_tag import parse_ctrl_flags

    ctrl = 0x03  # 两个位都设置
    flags = parse_ctrl_flags(ctrl)
    assert flags["overridable"] is True
...
def test_property_tag_ue5_11_serial_type():
    """UE5.11+ 的 SerialType 字段应被正确读取。"""
    from uasset_read.serializers.property_tag import read_property_tag_ue5
...
```python
# src/uasset_read/serializers/property_tag.py
...
def parse_ctrl_flags(ctrl: int) -> dict:
    ...
    ctrl & 0x02: 序列化控制扩展 (SerializeControl)
    """
    return {
        "overridable": bool(ctrl & 0x01),
...
```bash
git add src/uasset_read/serializers/property_tag.py tests/phase76/test_property_tag.py
