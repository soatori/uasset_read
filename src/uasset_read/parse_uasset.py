"""Main parse pipeline entry — parse_uasset() function.

Equivalent migration from uasset_read.py §6223-6412.
"""
...
def _parse_package_core(
    ...
    """Shared core parse logic — reads a package and populates result.
    ...
    Args:
        ...
        asset_roots: Asset root directories list
        extra_linker_setup: Additional callback after linker creation (linker, result) -> None
        check_aes_key: If provided, raise ParseError (parse_package compatibility)
            ...
            if result.graphs and result.export_map:
                for export in result.export_map:
                    ...
                    if name.endswith("_C") and not name.startswith("Default__"):
                        ...
                        break  # Assign only to main blueprint export
...
def parse_package(
    ...
    """
    Main entry point: parse an Unreal package (.uasset or .umap).

    Args:
        path: .uasset/.umap file path
        tolerant: Whether to enable tolerant mode (default True)
        ...
        force_full_parse: Force full parse for large blueprints (ignore lightweight mode threshold)
        hex_view: Enable HexView byte offset tracking
        config: Optional ParseConfig instance for centralized parse parameter management.
            When config is provided, legacy individual parameters can still override
    ...
    if include_linker is not True:
        warnings.warn(
            "include_linker parameter is deprecated; linker is always included in the result. "
...
def parse_uasset(
    ...
    """
    Legacy entry point: parse a .uasset file.
...
def parse_uasset_with_linker(
    ...
    """Parallel parse entry point using PackageLinker (D-01, D-04).
...
def parse_package_lazy(
    ...
    """Lazy-load mode package parsing — parse export bodies on demand.

    Always parses: Header, NameMap, ImportMap, ExportMap (metadata).
    Only parses body for specified export_indices; unspecified exports are marked as unloaded.

    When provider offers open_file(), it is preferred for obtaining the archive (supports mmap
    range reads), avoiding reading the entire file into memory; otherwise falls back to read_file() path.

    Args:
        path: .uasset/.umap file path
        export_indices: List of export indices whose body should be parsed; None means skip all
        store_raw_bytes: Whether to store raw export body bytes in lazy_load_archive
            (default False — lazy loading does not cache raw bytes to save memory)
    ...
    # open_file() supports mmap range reads, suitable for lazy loading.
    use_direct_archive = (
    ...
    with memory_monitor:
      try:
        ...
        if use_direct_archive:
            # Fast path: obtain archive via open_file(), without reading the entire file
        ...
        # Parse specified export body on demand
        ...
        for idx, export in enumerate(result.export_map or []):
            ...
            # Set lazy-load flag (using setattr for compatibility with ObjectExport and ExportIR)
