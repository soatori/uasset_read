...
from uasset_read.project_logging import log_context, log_event
...
def _read_core_tables(
    ...
    log_event(logging.getLogger(__name__), logging.INFO, "core_tables_start", asset=asset_name)
    # 读取文件头
    ...
    asset_name = Path(path).stem
    with log_context(asset=asset_name, stage="core_tables"):
        log_event(
    ...
    log_event(logging.getLogger(__name__), logging.INFO, "core_tables_done",
              asset=asset_name,
              names=len(result.name_map or []),
              imports=len(result.import_map or []),
              exports=len(result.export_map or []))
...
def _read_secondary_tables(
    ...
    asset_name = Path(path).stem
    with log_context(asset=asset_name, stage="secondary_tables"):
        ...
        if hasattr(result.summary, "preload_dependency_count"):
            result.summary.preload_dependencies = read_preload_dependencies(
                archive, result.summary
            )

        # 读取 SoftPackageReferences（软包引用表）
        if (
            hasattr(result.summary, "soft_package_references_count")
            and result.summary.soft_package_references_count > 0
        ):
            result.soft_package_references = read_soft_package_references(
                archive, result.summary, result.name_map
            )

        # 读取 SoftObjectPathList（UE5.7+ 用于索引化 SoftObjectProperty 解析）
        if (
            hasattr(result.summary, "soft_object_paths_count")
            and result.summary.soft_object_paths_count > 0
        ):
...
def _parse_export_properties(
    ...
    with log_context(stage="export_properties"):
            ...
            if export.serial_size > 0:
                try:
                    ...
                    if not getattr(export, "parse_status", None):
                        setattr(
                            export, "parse_status", validate_parse_status("success")
                        )
                    elif getattr(export, "parse_status", None) in (
                        "opaque",
                        "partial_metadata",
                    ):
                        pass
