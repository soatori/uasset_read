    ...
    *,
    ...
    try:
        ...
        with log_context(stage=stage):
            logger.exception("Unexpected error in stage %s: %s", stage, e)
        _record_parse_stage_error(result, archive, path, stage, field, e)
        return None


def _derive_package_name(path: str, summary) -> None:
    """Derive package_name from file path when empty."""
...
def _init_parse_env(
    ...
    Returns (archive, bundle, mappings_provider) or None on early-return conditions.
    """
...
def _read_core_tables(
    ...
    Returns True on success; returns False on early return (stage failure).
    """
    # Read file header
...
def _read_secondary_tables(
    ...
    # Read SoftObjectPathList (UE5.7+ indexed SoftObjectProperty parsing)
...
def _parse_export_properties(
    ...
    """Parse ExportMap properties -- dispatched via linker.preload()."""
    # Lazy import extras module (per #117 core/extras layering)
...
def _read_package_headers(
    ...
    """Read package file headers (Summary + NameTable + ImportMap + ExportMap + Linker).

    Reuses _init_parse_env + _read_core_tables, additionally creates linker.

    Returns:
        (bundle, archive, linker, mappings_provider) -- caller responsible for closing archive.
