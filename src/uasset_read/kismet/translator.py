...
class KismetTranslator:
    ...
    def line_cpp(self, expr: KismetExpression) -> str:
        ...
        # --- Variables ---
        if isinstance(expr, (EX_LocalVariable, EX_InstanceVariable, EX_DefaultVariable,
                             EX_LocalOutVariable, EX_ClassSparseDataVariable)):
        ...
        if isinstance(expr, EX_FloatConst):
            val = expr.Value
            return f"{val}f" if '.' not in str(val) else f"{val}f"
        ...
        if isinstance(expr, EX_TextConst):
            # EX_TextConst has a nested SourceString (EX_StringConst) or LOCTEXT format
        ...
        if isinstance(expr, EX_TransformConst):
            v = expr.Value
            return (f"FTransform(FQuat({v.Rotation.X}, {v.Rotation.Y}, {v.Rotation.Z}, {v.Rotation.W}), "
                    f"FVector({v.Translation.X}, {v.Translation.Y}, {v.Translation.Z}), "
        ...
        if isinstance(expr, (EX_Nothing, EX_NothingInt32, EX_EndOfScript,
            ...
            return ""
        # --- RTFM (must be checked before deprecated fallback) ---
        ...
        if isinstance(expr, EX_Return):
            ...
            if ret_expr is None:
                return "return"
            # Check if return expression is EX_Nothing
        ...
        if isinstance(expr, EX_JumpIfNot):
            cond = self.line_cpp(expr.BooleanExpression) if hasattr(expr, 'BooleanExpression') and expr.BooleanExpression else "?"
        ...
        if isinstance(expr, EX_ComputedJump):
            var = self.line_cpp(expr.CodeOffsetExpression) if hasattr(expr, 'CodeOffsetExpression') and expr.CodeOffsetExpression else "?"
            return f"goto {var};"
        ...
        if isinstance(expr, EX_PopExecutionFlow):
            # Without execution flow stack, output return as fallback
            return "return;"
        ...
        if isinstance(expr, EX_Cast):
            ...
            conversion = expr.ConversionType if hasattr(expr, 'ConversionType') else None
            if conversion is not None:
                ...
                cpp_type = type_map.get(conversion, "auto")
            else:
                cpp_type = "auto"
        ...
        if isinstance(expr, (EX_MetaCast, EX_DynamicCast, EX_ObjToInterfaceCast,
            ...
            target = self.line_cpp(expr.Target) if hasattr(expr, 'Target') else "?"
            class_ptr = expr.ClassPtr if hasattr(expr, 'ClassPtr') else None
        ...
        if isinstance(expr, (EX_Context, EX_Context_FailSilent)):
            obj = self.line_cpp(expr.ObjectExpression) if hasattr(expr, 'ObjectExpression') and expr.ObjectExpression else "?"
            ctx_expr = self.line_cpp(expr.ContextExpression) if hasattr(expr, 'ContextExpression') and expr.ContextExpression else ""
            # Extract function name from context expression result
        ...
        if isinstance(expr, EX_InterfaceContext):
            return self.line_cpp(expr.InterfaceValue) if hasattr(expr, 'InterfaceValue') and expr.InterfaceValue else "?"
        if isinstance(expr, EX_StructMemberContext):
            prop = str(expr.Property) if hasattr(expr, 'Property') else "?"
            struct = self.line_cpp(expr.StructExpression) if hasattr(expr, 'StructExpression') and expr.StructExpression else "?"
        ...
        if isinstance(expr, EX_LetValueOnPersistentFrame):
            assignment = self.line_cpp(expr.AssignmentExpression) if hasattr(expr, 'AssignmentExpression') and expr.AssignmentExpression else "?"
        ...
        if isinstance(expr, (EX_FinalFunction, EX_CallMath)):
            ...
            if hasattr(expr, 'Parameters') and expr.Parameters:
                for param in expr.Parameters:
                    ...
                    if p_str:  # skip empty (EX_EndFunctionParms etc.)
                        params_list.append(p_str)

            # StackNode is an int (FPackageIndex). Without linker context,
            # use Function_{index} placeholder (per PLAN.md Risk mitigation).
            stack_node = expr.StackNode if hasattr(expr, 'StackNode') else 0

            # EX_CallMath always goes through MathFunctionCleaner
            if isinstance(expr, EX_CallMath):
                # Without linker, we can't resolve StackNode to ClassName:FuncName
                # Use placeholder format
                class_name = f"Function_{stack_node}" if isinstance(stack_node, int) else str(stack_node)
                func_name = f"Call_{stack_node}"
                # Try to extract from Parameters if first param looks like a class reference
            ...
            # EX_FinalFunction: generic format
            return f"Function_{stack_node}({', '.join(params_list)})"

        if isinstance(expr, (EX_VirtualFunction, EX_LocalVirtualFunction)):
            func_name = expr.VirtualFunctionName.Text if hasattr(expr, 'VirtualFunctionName') and hasattr(expr.VirtualFunctionName, 'Text') else str(getattr(expr, 'VirtualFunctionName', '?'))
            params_list = []
            if hasattr(expr, 'Parameters') and expr.Parameters:
                for param in expr.Parameters:
                    p_str = self.line_cpp(param)
        ...
        if isinstance(expr, EX_InstanceDelegate):
            fn = expr.FunctionName if hasattr(expr, 'FunctionName') else "?"
            return f'"{fn}"'

        # --- Containers ---
        if isinstance(expr, (EX_SetArray,)):
            var = self.line_cpp(expr.AssigningProperty) if hasattr(expr, 'AssigningProperty') and expr.AssigningProperty else "?"
            values = []
        ...
        if isinstance(expr, EX_StructConst):
            struct_name = str(expr.Struct.Name) if hasattr(expr, 'Struct') and hasattr(expr.Struct, 'Name') else "Struct"
            values = []
            if hasattr(expr, 'Properties') and expr.Properties:
                for prop in expr.Properties:
                    v = self.line_cpp(prop)
                    if v:
                        values.append(v)
        ...
        if isinstance(expr, EX_ClearMulticastDelegate):
            delegate = self.line_cpp(expr.DelegateToClear) if hasattr(expr, 'DelegateToClear') and expr.DelegateToClear else "?"
        ...
        if isinstance(expr, EX_RemoveMulticastDelegate):
            delegate = self.line_cpp(expr.Delegate) if hasattr(expr, 'Delegate') and expr.Delegate else "?"
            to_remove = self.line_cpp(expr.DelegateToAdd) if hasattr(expr, 'DelegateToAdd') and expr.DelegateToAdd else "?"
            sep = "->" if isinstance(expr.Delegate, (EX_Context, EX_Context_FailSilent)) else "." if hasattr(expr, 'Delegate') and expr.Delegate else "."
            return f"{delegate}{sep}RemoveDelegate({to_remove})"

        # --- SwitchValue ---
        if isinstance(expr, EX_SwitchValue):
            index = self.line_cpp(expr.IndexTerm) if hasattr(expr, 'IndexTerm') and expr.IndexTerm else "?"
            if hasattr(expr, 'Cases') and len(expr.Cases) == 2:
                ...
                return f"{index} ? {case1} : {case0}"
            # Multi-case: switch/case
            lines = [f"switch ({index}) {{"]
            if hasattr(expr, 'Cases'):
                for case_item in expr.Cases:
                    case_idx = self.line_cpp(case_item.CaseIndexValueTerm) if hasattr(case_item, 'CaseIndexValueTerm') else "?"
