...
def read_fstring(stream: BinaryIO, version: int = 0) -> str:
    ...
    if version >= PakFileVersion.Utf8PakDirectory:
        ...
        if length > MAX_FSTRING_LENGTH:
            raise ParseError(f"FUtf8String length {length} exceeds maximum {MAX_FSTRING_LENGTH}")
    ...
    if length < 0:
        ...
        if len(data) < utf16_len:
            raise ParseError(f"UTF-16 string truncated: read {len(data)} < expected {utf16_len} bytes")
        stream.read(2)  # null terminator (2 bytes for UTF-16)
        return data.decode("utf-16-le", errors="replace").rstrip("\x00")
    else:
        # ANSI / UTF-8
        if length > MAX_FSTRING_LENGTH:
            raise ParseError(f"ANSI string length {length} exceeds maximum {MAX_FSTRING_LENGTH}")
        data = stream.read(length)
        if len(data) < length:
            raise ParseError(f"ANSI string truncated: read {len(data)} < expected {length} bytes")
        stream.read(1)  # null terminator
...
class FPakInfo:
    ...
    def deserialize(cls, stream: BinaryIO, file_size: int) -> "FPakInfo":
        ...
        for group in version_groups:
            ...
            magic_offset_in_trailer = 17 if group[0] >= PakFileVersion.EncryptionKeyGuid else 1
